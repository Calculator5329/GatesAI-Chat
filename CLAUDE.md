...
Notes:
- `npm run test:models` is a LIVE OpenRouter suite. It needs a key
  and costs money, so never run it as a routine gate.
...
- **Respect the layer boundaries** (UI → stores → services → core). ESLint
  enforces them, so never weaken `eslint.config.js` to make an import work.
...
- **Never commit secrets.** API keys live in the OS keychain
  ...
  fixtures, or commit messages. `.env` is untracked, so keep it that way.
...
- Release asset names are stable and deep-linked from README/app, so never
  rename them.
