...
- **Bring-your-own-model chat** — OpenRouter cloud catalog (live `/api/v1/models` refresh,
  ...
  On desktop without an OpenRouter key, a detected online Ollama model becomes the default for
  a fresh untouched chat; GatesAI preserves every explicit model choice. Otherwise new chats use
  **Nemotron 3 Ultra free** (`nvidia/nemotron-3-ultra-550b-a55b:free` via OpenRouter, $0 but
  free-tier rate-limited). Paste your OpenRouter API key under **Menu → Models → OpenRouter →
  Connect** when you choose cloud chat. Web Lite remains cloud-only.
