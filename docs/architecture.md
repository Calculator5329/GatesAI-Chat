...
Readable library write reuse: the workspace persistence instance retains each successful rendered HTML/Markdown pair and its write timestamp. A complete file listing and unchanged bridge connection epoch allow exact re-render comparison to skip unchanged pairs; the global index remains fresh. Reconnects and new workspace persistence instances invalidate reuse. Unknown bridge facades are uncached. Failed/incomplete listing falls back to full writes without cache promotion; partial writes clear all cached entries. Per-conversation Saved is the last successful pair write, not the latest global snapshot. The cache retains rendered strings in memory and targets bridge writes, not render CPU. File presence is not integrity proof: external edits are never imported and can remain until a changed projection or lifecycle invalidation rewrites the current path.

### Autosave observation scheduling (A53, 2026-09-05)

The chat persistence coordinator applies its existing 250 ms leading/trailing throttle to MobX reaction execution, including deep dependency observation, rather than only to the subsequent write. Initial registration/save remains immediate. Pending reactions read the latest structural and nested snapshot, and pagehide/beforeunload/dispose drain them before synchronous persistence flush or reaction disposal. Workspace queue serialization and field observation are unchanged. See `design-autosave-scheduling-20260905.md` for measured scope and verification status.

The separately adopted A54 pause rule clears pending workspace snapshots and blocks private enqueue/drain while paused. Resume only re-enables scheduling, allowing the caller to refresh leader state before saving. Already-started writes and direct hydration saves are outside this coordinator-owned suppression.
