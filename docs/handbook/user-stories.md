...
| --- | --- | --- |
| P0 | As a Web Lite visitor, I want to try smooth chat, switch between tested LLMs, and see browser-safe inline artifacts so I understand the product before installing. | Web Lite accepts an OpenRouter key, allows supported model switching, can show web-safe HTML artifacts when available, hides bridge-only tools, and links clearly to Desktop. |
...
| P1 | As a user choosing a model, I want a trusted default and a tested model list so I do not need to understand the whole catalog first. | Model picker highlights the best default, separates tested models from full catalog options, supports switching between tested model families when compatible, and labels tool/vision/reasoning/cost capabilities. |
...
| P1 | As a workspace user, I want the assistant to work in a safe folder that stays mostly hidden until relevant. | Workspace is seeded with useful folders/docs, initialized as git when possible, surfaced when the assistant uses it or in settings, and never auto-commits user artifacts by default. |
...
| P2 | As a customization user, I want a future plugin/addition system so GatesAI can grow around my workflow. | Built-in internal capabilities remain standard, while future plugin boundaries can add new tools/workflows without scattering provider/tool logic. |
| P2 | As a self-improvement user, I want GatesAI to edit its bundled source copy, build a new EXE, and let me approve installation. | Source workspace flow separates source-copy edits from the live app, runs tests/builds, produces a new executable, shows results, and requires explicit user install/update approval. |
