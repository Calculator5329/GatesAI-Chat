...
import { assistantMessageParts, userMessageParts } from './core/messageParts';
    ...
    messages: [
      {
        ...
        role: 'assistant',
        parts: assistantMessageParts({ text: 'Welcome to GatesAI Chat. This short, read-only tour uses the same message surfaces as a normal conversation so you can see how tools, workspace artifacts, and images appear.' }),
        createdAt: now,
        model: modelId,
      },
      {
        ...
        role: 'user',
        parts: userMessageParts('What is already in my workspace?'),
        createdAt: now + 1_000,
      },
      {
        ...
        parts: assistantMessageParts({
          text: 'I checked the workspace first. Tool activity stays with the reply, and longer output is collapsed until you want to inspect it.',
          toolCalls: [{
      ...
      },
      {
        ...
        role: 'assistant',
        parts: assistantMessageParts({
          text: 'Artifacts are durable deliverables such as reports, apps, and datasets. This is a static tour representation of an HTML artifact—no file was created for this bundled example.',
          toolCalls: [{
      ...
      },
      {
        ...
        role: 'user',
        parts: userMessageParts('Here is an image attachment.', [{
      ...
      },
      {
        ...
        role: 'assistant',
        parts: assistantMessageParts({ text: 'Image attachments appear directly on the message. This one is a static placeholder, so it does not require a bundled image file; your pasted, dropped, or attached images render here normally.' }),
        createdAt: now + 5_000,
        model: modelId,
      },
      {
        ...
        role: 'assistant',
        parts: assistantMessageParts({ text: 'When you are ready, start a new conversation. Use ↑ in the composer to recall a previous prompt, and paste or drag files and images straight into the window to attach them.' }),
        createdAt: now + 6_000,
        model: modelId,
      },
    ],
